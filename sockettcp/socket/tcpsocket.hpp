...
class TcpSocket
{
    public:
        ...
        bool Bind(const std::string &ip, uint16_t port) {
            ...
            Addr(&addr, ip, port);//给socket添加地址信息
        ...
        //客户端发起连接请求，客户端虽然不用绑定本机ip地址，但是发起连接请求的时候需要对端ip和port
        ...
        //获取新建连接，因为要返回新建套接字的句柄所以，将新对象直接传入，得到新的句柄
        bool Accept(TcpSocket *sock, std::string *ip = NULL, uint16_t *port = NULL) {
            //accept(监听套接字描述符，客户端地址信息，地址信息长度)返回操作句柄
            //返回的操作句柄赋值到传入的sock的成员sockfd中，通过sock可以与客户端通信
            struct sockaddr_in cliaddr;//操作系统新建的socket将五元组已经放好，新的socket被返回句柄
            socklen_t len = sizeof(struct sockaddr_in);
