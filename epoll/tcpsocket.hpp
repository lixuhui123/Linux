...
class TcpSocket
{
    public:
        ...
        void SetNonBlock() {
            //fcntl 获取文件属性/设置文件属性
            int flag = fcntl(_sockfd, F_GETFL, 0);//获取文件已有属性
            fcntl(_sockfd, F_SETFL, flag | O_NONBLOCK);//在原有属性基础上增加非阻塞
        }
        //创建套接字
        bool Socket() {
        ...
        bool Recv(std::string *buf) {
            ...
            while(1) {//循环将缓冲区中的数据全部读取出来，
                char tmp[5] = {0};
                ssize_t ret = recv(_sockfd, tmp, 5, 0);//若没有数据就会阻塞
                if (ret < 0) {
                    if (errno == EAGAIN) {
                        //表示非阻塞下，缓冲区中没有数据
                        return true;//把数据取完了
